...
#data = [d for d in tweetdata.find({'emo_val':{ "$exists": True }},{'noun':1,'adjective':1,'verb':1,'adverb':1})] #一度目では動かないコードだったので下記に変更
data = [d for d in tweetdata.find({},{'noun':1,'adjective':1,'verb':1,'adverb':1,'emo_val':1})]
...
for d in data:
    ...
    if type(d.get('emo_val')) is float or type(d.get('emo_val')) is int: #解析済みはスキップ
#         print('スキップ')
        continue
    score = 0
    word_count = 0
