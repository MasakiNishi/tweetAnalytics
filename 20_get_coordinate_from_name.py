...
import requests
import codecs
with codecs.open("timei-all.txt",'r','shift_jis') as f: #ファイル名を上記サイトからダウンロードしたデフォルトのままに
...
res2 = np.array(list(map(len,res)))
loc_list_in_tweet = np.unique(res[res2>0])

def get_coordinate_from_location(location_name):
    payload = {'appid': '******************************', 'output':'json'}
    #YahooJapanAPIのClient IDが必要。置き換えhttps://e.developer.yahoo.co.jp/dashboard/
    ...
    if r.status_code == 200:
        ...
        try:
            ret = np.array([list(map(float,j['Geometry']['Coordinates'].split(','))) for j in jdata['Feature']])
...
w_list = [loc for loc in location_dict.find({})]
